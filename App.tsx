...
function AppContent() {
  ...
  async function onLogin() {
    ...
    try {
      ...
      if (existing.length) {
        syncHistoryToPlaylist(t, [...existing].reverse().map((e) => e.uri)).catch((e) =>
  ...
  const onRoll = useCallback(async () => {
    if (!token?.accessToken) {
      Alert.alert('Connect Spotify first', 'Tap "Connect Spotify" to continue.');
      return;
    }
    ...
    try {
      ...
      await syncHistoryToPlaylist(fresh, updated.map((e) => e.uri)).catch((e) =>
        console.warn('Playlist sync failed:', e),
      );
  ...
  useEffect(() => {
    requestWidgetUpdate({ widgetName: 'DiceWidget', renderWidget: () => <DiceWidget /> }).catch(() => {});
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.connectContainer}>
          <Image source={require('./assets/dice-logo.png')} style={styles.logo} />
          <Text style={styles.title}>Dice Radio</Text>
          <Text style={styles.subtitle}>Chaotic Spotify discovery</Text>
          <TouchableOpacity style={styles.connectButton} onPress={onLogin} disabled={authBusy}>
            {authBusy ? (
      ...
      <View style={styles.historyContainer}>
        {history.length > 0 ? (
          ...
          <View style={styles.emptyHistory}>
            ...
            <Text style={styles.emptySubtext}>Hit ROLL to discover something chaotic</Text>
          </View>
        )}
      </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <TouchableOpacity style={styles.settingsRow} onPress={onRefreshPlaylist} disabled={refreshing}>
              {refreshing ? (
              ...
              <View style={styles.settingsRowContent}>
                ...
                <Text style={styles.settingsRowHint}>Check if "Chaos Calls" exists, or create it</Text>
              </View>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.settingsRow} onPress={onLogout}>
              <Text style={styles.settingsRowIcon}>🚪</Text>
              <View style={styles.settingsRowContent}>
                <Text style={[styles.settingsRowText, { color: '#ff4d4d' }]}>Disconnect Spotify</Text>
                <Text style={styles.settingsRowHint}>Sign out and clear session</Text>
              </View>
            </TouchableOpacity>

            <TouchableOpacity style={styles.closeModal} onPress={() => setSettingsOpen(false)}>
