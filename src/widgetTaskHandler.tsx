...
import { fetchChaoticTrack, queueRelatedTracks, startPlayback, syncHistoryToPlaylist } from './spotify';
...
registerWidgetTaskHandler(async ({ renderWidget, widgetAction }) => {
  if (widgetAction === 'WIDGET_CLICK') {
    ...
    try {
      ...
      await syncHistoryToPlaylist(token, history.map((e) => e.uri)).catch(() => {});
